		...
		if (!m_glinfo.isGLES2) {
			...
			if (CombinerProgramBuilder::s_textureConvert.useTextureFiltering()) {
				...
				shaderPart +=
					...
					"  } else {															\\\n"
					"    if (uTextureFilterMode == 0) {TEX_NEAREST(name, tex, tcData);}	\\\n"
	...
	ShaderMipmap(const opengl::GLInfo & _glinfo)
	{
		if (_glinfo.isGLES2) {
			...
			static const std::string strReadTex1 =
				...
				"  mediump vec4 texWdthAndOff = 255.0 * texelFetch(tex, ivec2(2*int(tile), 0), 0);		\\\n"
				"  mediump vec4 texScale = 255.0 * texelFetch(tex, ivec2(2*int(tile) + 1, 0), 0);		\\\n"
				"  mediump vec2 lod_scale = vec2(1.0f) / texScale.rg;									\\\n"
